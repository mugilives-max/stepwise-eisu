...
GAS現行版はv51。授業記録と月間承認・請求、授業形式と同時人数、案内の一括確定、兄弟姉妹の家族アカウント、保護者メール通知は公開済み。今回の追加機能もGAS・Pagesへ反映し、先生・本人画面の対象操作、テストデータの後片付けと専用リンクの失効を確認した。版ごとの反映範囲と確認結果は [6章](#6-更新デプロイ手順)、運用上の残件はFUTURE_WORKを参照する。
...
## ローカル実装: 取消申請・成績票・連絡欄（2026-09-09、未公開）

本節は未反映の追加仕様。公開版の記録は6章のv52のまま。実装の正本は `gas/LearningServices.gs`、共通画面は `assets/learning-services.js`。管理画面ホームに連絡受付件数と受信箱、生徒の個別ページに成績票・連絡、生徒・保護者のページに公開資料・連絡・取消申請を追加した。パネルは開いた時に取得する。
...
公開時は2台帳と現行ソースを退避・比較してから、新規シートの列を確認する。新しいGASファイルとCode.gs、共通JSと管理/予約HTMLを一括の反映対象とし、GASを先に反映する。一時的な検証・退避用のエディタファイルは公開前に削除する。既存の `setup()` を再実行しない。Drive権限の追加同意が必要なら先生本人が行い、非公開のテストPDFで実Driveの保存・取得・他生徒からの拒否を確認する。今回その操作もデプロイも実施していない。

ローカル検証: 構文検査と410件の自動テストに合格。新機能は認証、家族解除、非公開欄、PDFの再送、返信競合、24時間境界と処理待ち、承認/却下を検証した。隔離したブラウザ画面で成績コメント保存・本人表示、連絡送信・先生の返信・本人側の表示、取消申請を確認。実Driveの権限とPDF配信は本番反映時の残件。
