...
| URL | 誰が | 中身 |
|---|---|---|
| `/yoyaku/?k=<code>` | 生徒(LINEで専用リンクを配布) | マイページ(ヘッダー左上は「〇〇さんのマイページ」)。タブ: ホーム(予定表(登録不可の日・時間帯も表示)・選んだ日の予定(＋で授業可能日時・授業できない日・予定の共有を登録。登録済みの授業可能日時の取消・授業できない日の解除・予定の削除も同じ場所)。＋を押すと「文章で予定入力」も出る(AI 変換→確認→登録。鍵設定時のみ))・今月の授業・やること・授業登録の確定/再調整)/ 成績(模試・成績推移)/ 授業の記録（公開された内容・宿題） / メール通知（登録・受信確認・解除） / 保護者(パスワード制。今月の授業・お支払い・授業回数の承認) |
