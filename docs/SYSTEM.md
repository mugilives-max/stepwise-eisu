...
| URL | 誰が | 中身 |
|---|---|---|
| `/yoyaku/?k=<code>` | 生徒(LINEで専用リンクを配布) | マイページ(ヘッダー左上は「〇〇さんのマイページ」)。タブ: ホーム(予定表(登録不可の日・時間帯も表示)・選んだ日の予定と登録・予定管理: 授業可能日時・予定の共有・授業できない日・今月の授業・やること・授業登録の確定/再調整)/ 成績(模試・成績推移)/ 授業の記録（公開された内容・宿題） / メール通知（登録・受信確認・解除） / 保護者(パスワード制。今月の授業・お支払い・授業回数の承認) |
