...
GAS現行版はv55。授業記録と月間承認・請求、授業形式と同時人数、案内の一括確定、兄弟姉妹の家族アカウント、保護者メール通知は公開済み。今回の追加機能もGAS・Pagesへ反映し、先生・本人画面の対象操作、テストデータの後片付けと専用リンクの失効を確認した。版ごとの反映範囲と確認結果は [6章](#6-更新デプロイ手順)、運用上の残件はFUTURE_WORKを参照する。
...
### ローカル実装: 授業情報の集約・実施分請求・家族明細

2026-09-09の第一段階。本番はv55のまま。以下は作業ブランチの未公開仕様である。
...
検証: 全体430件通過後、最終変更に関する44件を再確認（家族明細の認証境界テストを追加）。構文確認、架空データによる共通画面のブラウザ表示も確認。本番のデータ・認証・請求・トリガーは変更していない。

**保護者の既読（追加ローカル実装、本番未反映）**: 確認印・確認ボタンは作らず、保護者の授業記録一覧に「未読／既読／更新あり」を表示する。記録の折りたたみを開いた時だけ `learningService` の `recordRead` を呼び、一覧取得は `recordReadStatus` の読取のみ。生徒画面は既読を書き込まない。
...
追加後の構文確認と全436件のテストは成功（既読API・再送・更新・認証範囲・画面の開閉/失敗/遅延応答を含む）。公開時は既存台帳の退避後に `serviceEnsure_('lessonReadReceipts')` で新規表を準備し、`assets/lesson-read.js` を含めてGAS→画面の順に反映する。実GASでの確認は公開作業時に行う。

公開時は最新の2台帳を退避・照合後に `ensureLessonSchema_` の末尾追加を実GASで検証する。途中の既存ジャーナル・既存列の保持も確認する。v55に対応するGitコミットを基準にGASを反映し、`assets/lesson-report.js` / `.css` と2画面をGASの後で公開する。表示や保存の実GAS確認、旧固定月謝データの影響確認はまだ実施していない。
