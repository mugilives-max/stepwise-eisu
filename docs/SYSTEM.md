...
公開時は2台帳と現行ソースを退避・比較してから、新規シートの列を確認する。新しいGASファイルとCode.gs、共通JSと管理/予約HTMLを一括の反映対象とし、GASを先に反映する。既存の `setup()` を再実行しない。Drive権限の追加同意が必要なら先生本人が行い、非公開のテストPDFで実Driveの保存・取得・他生徒からの拒否を確認する。今回その操作もデプロイも実施していない。
