...
ローカル検証は構文チェックと368件の回帰テストを通過。準備の非公開・所有者分離・版競合・中断再送、実施未登録の抽出、画面分割後の設定・請求操作を含む。実際のブラウザでの表示・実GASでの検証は含まない。

公開時には既存の反映手順に従い、予約台帳の `lessonPreparations` 新規シートと既存データ保持を確認する。スキーマキャッシュキーは `schemaOk18`。本番の構造準備・実GAS検証・画面確認は未実施。
