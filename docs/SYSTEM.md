...
- 生徒画面の新規登録は「授業可能日時」に統一し、開始・終了時刻の範囲を登録する（API上は `kind=ok`）。ホーム内でも登録できる。授業形式は生徒設定から引き継ぎ、希望の種類・形式の選択欄は表示しない。範囲すべての授業を希望・確定する意味ではない。従来の日時指定データとAPI互換性は保持する。送信前に空き状況を確認する。満員でも送信でき、先生側には送信時の要調整状態を表示する。単日・複数日とも対応し、形式と必要な授業時間を指定する。日時指定はその区間全体、幅のある希望は中に連続した授業時間を確保できるか確認する。返事待ちの案内、確定授業、変更処理中の移動先、先生の休み・本人の授業不可を考慮する。他生徒の氏名・ID・予定明細は返さない。
