...
GAS現行版はv49。授業記録と月間承認・請求、授業形式と同時人数、案内の一括確定、兄弟姉妹の家族アカウント、保護者メール通知は公開済み。今回の追加機能はGASへ反映済みで、Pagesへの画面反映と公開画面の操作確認はこれから行う。版ごとの反映範囲と確認結果は [6章](#6-更新デプロイ手順)、運用上の残件はFUTURE_WORKを参照する。

案内編集・予定表の時刻順表示・化学の選択・生徒メール・授業記録の保存時公開・次回授業を使う宿題期限は、実GAS検証と本番の構造準備を完了した。**GAS v49は公開済み。追加画面のPages反映と操作確認は未完了。** 3〜5章はこの追加実装を含む仕様を記載し、公開状況と完了した検証は [6-3節](#6-3-v49の反映と検証) に集約する。
...
**公開状況: GAS v49は公開済み。追加画面のPages反映と公開画面の操作確認は未完了。** 実装・実GAS検証・本番の構造準備は完了した。
...
- ローカル検証は全体352件の通過後、追加した3件も通過し、重複を除く355件を確認した。実GASでは隔離した架空台帳を使い、授業案内・形式・一括確定53項目、授業記録29項目、生徒メール36項目の計118項目がすべて通過した。これは公開後の画面操作確認の代わりにはしない。
...
- 証跡と除去済みの一時検証・移行・訂正関数は非公開の `.verification/learning-20260908/` に保存する。残る確認はPagesへの反映と配信HTMLの照合、対象画面の操作確認、テストデータの後片付け。結果を確認後にこの節へ追記する。
