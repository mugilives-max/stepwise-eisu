...
1. **本番の保護者ログイン**: 専用テスト家族の `familyLogin` が汎用エラーを返したため、実環境の家族セッションによる報告・既読確認は未完了。登録情報を使った同じ処理は、退避した本番DB＋テスト行のローカルコピーでは成功した。今回、既存の認証方式・パスワード処理は変更していないが、本番固有の例外原因と変更前からの発生有無は未確定。テスト家族は削除済み。実アカウントのパスワード変更・再設定や保護条件の緩和で回避しない。次の作業として原因調査・修正の範囲を先生と確認する。
