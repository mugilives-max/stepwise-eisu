...
LESSON_HEADERS_.lessonPublicSnapshots.push('reportJson');

// Optional public report details share the existing record revision/journal.
function lessonReportFields_(input) {
  var fields=['teacher','plannedUnit','actualUnit','understanding','pace','homeworkReview','homeworkAccuracy','testDate','parentMessage'];
  lessonOnly_(input,fields);var out={};
  fields.forEach(function(k){out[k]=lessonText_(input[k],k==='parentMessage'?1000:200,false);});
  if(out.homeworkAccuracy && (!/^\d{1,3}(\.\d)?$/.test(out.homeworkAccuracy)||Number(out.homeworkAccuracy)>100))lessonFail_('validation','宿題の正答率は0〜100%で入力してください');
  out.testDate=lessonDate_(out.testDate);
  return out;
}
function lessonReportRead_(r) { return r && r.reportJson ? lessonReportFields_(JSON.parse(String(r.reportJson))) : {}; }
function lessonWorkspace_(studentId,reference) {
  var date=String(reference.lessonDate||reference.date),start=String(reference.lessonStart||reference.start),subject=String(reference.subject||''),key=date+'T'+start;
  var slots=readRows_('slots').filter(function(s){return String(s.studentId)===String(studentId)&&s.status==='booked'&&subject&&String(s.subject||'')===subject;}).sort(slotSort_);
  var month=slots.filter(function(s){return s.date.slice(0,7)===date.slice(0,7);}),next=slots.filter(function(s){return s.date+'T'+s.start>key;})[0];
  var index=month.findIndex(function(s){return s.date===date&&s.start===start&&String(s.id)===String(reference.slotId||reference.id);});
  var planned=planRows_().filter(function(p){return String(p.studentId)===String(studentId)&&String(p.ym)===date.slice(0,7)&&String(p.subject)===subject;});
  var report=lessonReportRead_(reference),beforeTest=report.testDate?slots.filter(function(s){return s.date+'T'+s.start>key&&s.date<=report.testDate;}).length:null;
  return {plannedCount:planned.length===1?Number(planned[0].count):null,scheduledCount:month.length,ordinal:index<0?null:index+1,next:next?{date:next.date,start:next.start,subject:String(next.subject),days:Math.round((new Date(next.date+'T00:00:00Z')-new Date(date+'T00:00:00Z'))/86400000)}:null,beforeTest:beforeTest};
}
...
function lessonPayload_(req) {
  ...
  } else if ((req.op === 'lessonPreparationSave' || req.op === 'lessonRecordDraftSave')) {
    p.slotId=lessonId_(req.slotId); p.expectedRevision=lessonInteger_(req.expectedRevision,0); p.body=lessonText_(req.body,req.op==='lessonRecordDraftSave'?30000:4000,false);
    if(req.op==='lessonRecordDraftSave'){var draft;try{draft=JSON.parse(p.body);}catch(err){lessonFail_('validation','一時保存の内容を確認してください');}if(!draft||!draft.form||typeof draft.form!=='object'||Array.isArray(draft.form)||!Number.isInteger(draft.baseRevision)||draft.baseRevision<0)lessonFail_('validation','一時保存の内容を確認してください');}
...
function lessonPlan_(p,requestId,hash) {
  ...
  if ((p.operation === 'lessonPreparationSave' || p.operation === 'lessonRecordDraftSave')) {
    ...
    if (['date','start','min','subject'].some(function(k){return String(slot[k] || '')!==String(p.expectedSlot[k] || '');})) lessonFail_('conflict','授業の日時・科目が変わっています。入力を残して最新を確認してください');
    var preparation=lessonFind_('lessonPreparations','id',(p.operation==='lessonRecordDraftSave'?'record-draft:':'')+lessonPreparationKey_(p.studentId,p.slotId));
    if (preparation && String(preparation.studentId)!==p.studentId) lessonFail_('notFound','準備の対象が一致しません');
    lessonRevision_(preparation,p.expectedRevision);
    if(p.operation==='lessonRecordDraftSave'){
      var published=lessonRows_('lessonRecords').filter(function(x){return String(x.studentId)===p.studentId&&String(x.slotId)===p.slotId&&x.status==='active';});
      if(published.length>1)lessonFail_('conflict','有効な授業記録が重複しています');
      lessonRevision_(published[0]||null,JSON.parse(p.body).baseRevision);
    }
  } else if (p.operation === 'lessonRecordSave') {
...
function lessonExecute_(plan) {
  ...
  if (plan.operation === 'planOutlineSave') {
  ...
  } else if ((plan.operation === 'lessonPreparationSave' || plan.operation === 'lessonRecordDraftSave')) {
    ...
    if (plan.publicSnapshot) {
      ...
      if (prior && LESSON_HEADERS_.lessonPublicSnapshots.some(function (key) { return String(prior[key] == null ? '' : prior[key]) !== String(plan.publicSnapshot[key] == null ? '' : plan.publicSnapshot[key]); })) lessonFail_('conflict','公開版の内容が一致しません');
...
function lessonPreparationView_(studentId,slotId) {
  ...
  return {body:String(p.body || ''),revision:Number(p.revision),updatedAt:String(p.updatedAt),date:String(p.lessonDate),start:String(p.lessonStart),min:Number(p.lessonMin),subject:String(p.subject || '')};
}
...
function lessonMetadata_(studentId,slotId) {
  if (!MEMO_.lessonMeta) {
    var map=Object.create(null),drafts=Object.create(null);
    lessonRows_('lessonReportDrafts').forEach(function(d){drafts[String(d.recordId)]=true;});
    lessonRows_('lessonRecords').forEach(function(r){
      var key=JSON.stringify([String(r.studentId),String(r.slotId)]);
      if (r.status==='active' && (!map[key] || map[key].lessonRecordStatus!=='active')) map[key]={lessonRecordStatus:'active',lessonDraftStatus:drafts[String(r.id)]?'draft':'none'};
      else if (!map[key]) map[key]={lessonRecordStatus:'void',lessonDraftStatus:'none'};
    });
    MEMO_.lessonMeta=map;
  }
  return MEMO_.lessonMeta[JSON.stringify([String(studentId),String(slotId)])] || {lessonRecordStatus:'none',lessonDraftStatus:'none'};
}

function lessonTemporaryDraft_(studentId,slotId) {
  var p=lessonFind_('lessonPreparations','id','record-draft:'+lessonPreparationKey_(studentId,slotId));
  if(!p||String(p.studentId)!==String(studentId))return null;
  try {var d=JSON.parse(p.body);return {form:d.form,baseRevision:d.baseRevision,revision:Number(p.revision),updatedAt:String(p.updatedAt)};}catch(e){return null;}
}
