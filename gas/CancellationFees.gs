...
function cancelAttendance_(sid){return cancelFeeRows_().filter(function(r){return r.status==='confirmed'&&(!sid||String(r.studentId)===String(sid));}).map(function(r){var d=JSON.parse(r.decisionJson),s=d.quote.slot;return {id:r.id,studentId:String(r.studentId),studentName:studentName_(r.studentId),date:s.date,start:s.start,min:Number(s.min),subject:s.subject,label:d.quote.source==='teacher'?'休講':'欠席',amount:d.amount,standardAmount:d.quote.amount,relief:d.relief||null,reason:d.note||d.requestReason||'',source:d.quote.source};});}

function cancelCalendar_(sid){return cancelFeeRows_().filter(function(r){return r.status==='confirmed'&&String(r.studentId)===String(sid);}).map(function(r){var d=JSON.parse(r.decisionJson),x=d.quote.slot;return {id:r.id,date:x.date,start:x.start,min:Number(x.min),subject:x.subject,st:'cancelled',amount:d.amount,standardAmount:d.quote.amount,receivedAt:d.quote.source==='noshow'?'':d.quote.receivedAt,source:d.quote.source,relief:d.relief||null,confirmed:!!d.acknowledgedAt};});}
function cancelAcknowledge_(req){var me=findStudentByCode_(req.k);if(!me)return {error:'専用リンクから開き直してください',badCode:true};var r=cancelFeeRows_().filter(function(x){return x.id===String(req.cancellationId)&&String(x.studentId)===String(me.id)&&x.status==='confirmed';})[0];if(!r)return {error:'キャンセル記録が見つかりません'};var d=JSON.parse(r.decisionJson),reason=String(req.reason||'').trim();if(reason.length>1000)return {error:'事情は1000文字以内で入力してください'};if(d.acknowledgedAt&&req.reason!==undefined&&reason!==String(d.acknowledgementReason||''))return {error:'すでに確認済みです。追加の事情は先生への連絡からお知らせください'};if(!d.acknowledgedAt){if(reason){var slot=d.quote.slot;serviceMessageSend_({requestId:'cancel-'+schedulingHash_(r.id),category:'schedule',body:'キャンセルについての事情：'+slot.date+' '+slot.start+' '+slot.subject+'\n'+reason},{student:me,role:req.familyProxy?'parent':'student',senderId:'cancel-'+me.id});}d.acknowledgementReason=reason;d.acknowledgedAt=new Date().toISOString();d.acknowledgedBy=req.familyProxy?'parent':'student';r.decisionJson=JSON.stringify(d);cancelFeeWrite_(r);}return {ok:true,state:studentState_(req.k)};}

function cancelReliefPending_(sid,ym){return cancelFeeItems_(sid).filter(function(x){return x.reliefPending&&x.date.slice(0,7)<=ym;});}
function cancelReliefRequest_(req){
 var me=findStudentByCode_(req.k);if(!me)return {error:'専用リンクから開き直してください',badCode:true};
 var r=cancelFeeRows_().filter(function(x){return x.id===String(req.cancellationId)&&String(x.studentId)===String(me.id)&&x.status==='confirmed';})[0];if(!r)return {error:'キャンセル記録が見つかりません'};
 var reason=String(req.reason||'').trim(),rid=serviceId_(req.requestId);if(!reason||reason.length>1000)return {error:'申請理由を1000文字以内で入力してください'};
 var d=JSON.parse(r.decisionJson);if(d.relief){if(d.relief.requestId===rid&&d.relief.reason===reason)return {ok:true,state:studentState_(req.k)};return {error:'このキャンセルは申請済みです。追加の事情は先生へご連絡ください'};}
 if(!(d.amount>0))return {error:'キャンセル料が0円のため申請は不要です'};
 d.relief={requestId:rid,reason:reason,status:'pending',requestedAt:new Date().toISOString(),requestedBy:req.familyProxy?'parent':'student',previousAmount:d.amount};
 r.decisionJson=JSON.stringify(d);cancelFeeWrite_(r);return {ok:true,state:studentState_(req.k)};
}
function cancelReliefReview_(req){
 var r=cancelFeeRows_().filter(function(x){return x.id===String(req.cancellationId)&&String(x.studentId)===String(req.studentId)&&x.status==='confirmed';})[0];if(!r)return {error:'キャンセル記録が見つかりません'};
 var d=JSON.parse(r.decisionJson),a=d.relief,note=String(req.note||'').trim(),amount=Number(req.amount);
 if(!a||a.requestId!==req.requestId)return {error:'申請内容が変わりました。画面を更新してください'};
 if(!note||note.length>1000||req.amount===undefined||req.amount===null||String(req.amount).trim()===''||!isFinite(amount)||amount<0||Math.floor(amount)!==amount||amount>a.previousAmount)return {error:'判断理由と、現在の料金以下の整数金額を入力してください'};
 if(a.status!=='pending'){if(a.amount===amount&&a.response===note)return {ok:true};return {error:'すでに審査済みです。画面を更新してください'};}
 if(billingInvoicedIds_(r.studentId).ids['cancel-fee:'+r.id])return {error:'この料金は請求済みです。請求を取り消してから審査を確定してください'};
 a.status=amount===a.previousAmount?'unchanged':amount===0?'waived':'reduced';a.amount=amount;a.response=note;a.decidedAt=new Date().toISOString();
 d.amount=amount;d.acknowledgedAt='';r.decisionJson=JSON.stringify(d);cancelFeeWrite_(r);return {ok:true};
}
