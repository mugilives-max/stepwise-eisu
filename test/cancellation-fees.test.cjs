...
test('cancelled calendar entries are visible without counting as booked or disclosing teacher notes',()=>{const h=fixture();request(h);const q=quote(h);ok(decide(h,q,{note:'先生専用メモ'}));const st=h.get({action:'state',k:'synthetic-link-a'});const c=st.cancellations||st.state&&st.state.cancellations;assert.equal(c.length,1);assert.equal(c[0].st,'cancelled');assert.equal(c[0].reason,undefined);assert.equal(c[0].amount,0);const cal=require('../assets/calendar.js');const info=cal.buildInfo({cancellations:c});assert.equal(info[c[0].date].mine,0);const html=cal.render(info,{year:2026,month:8,today:'2026-09-08'});assert.match(html,/calbox cancelled/);assert.match(html,/キャンセル済み/);});

test('cancellation acknowledgement is owned, persistent and idempotent',()=>{const h=fixture();request(h);ok(decide(h,quote(h)));const st=h.get({action:'state',k:'synthetic-link-a'}),id=st.cancellations[0].id;assert.equal(st.cancellations[0].confirmed,false);assert.ok(h.request({action:'cancelAcknowledge',k:'synthetic-link-b',cancellationId:id}).error);assert.ok(h.request({action:'cancelAcknowledge',cancellationId:id}).error);for(let i=0;i<2;i++){const r=ok(h.request({action:'cancelAcknowledge',k:'synthetic-link-a',cancellationId:id}));assert.equal(r.state.cancellations[0].confirmed,true);}assert.equal(h.rows('cancellationFees').length,1);});

test('optional cancellation circumstances reach teacher inbox only once without changing fee',()=>{const h=fixture();h.advance(10*3600000+1);request(h);ok(decide(h,quote(h)));const id=h.get({action:'state',k:'synthetic-link-a'}).cancellations[0].id;const req={action:'cancelAcknowledge',k:'synthetic-link-a',cancellationId:id,reason:'急病で連絡できませんでした'};assert.ok(h.request({...req,reason:'x'.repeat(1001)}).error);ok(h.request(req));ok(h.request(req));assert.equal(h.rows('contactMessages').length,1);assert.match(h.rows('contactMessages')[0].body,/急病/);assert.equal(h.rows('contactMessages')[0].status,'received');assert.equal(bill(h).amount,1000);assert.ok(h.request({...req,reason:'変更'}).error);});

function relief(h,id,reason='急病'){return h.request({action:'cancelReliefRequest',k:'synthetic-link-a',cancellationId:id,requestId:'relief-request-01',reason});}
test('relief after acknowledgement holds billing, teacher review releases reduced fee and resets acknowledgement',()=>{const h=fixture();h.advance(10*3600000+1);request(h);ok(decide(h,quote(h)));const id=h.get({action:'state',k:'synthetic-link-a'}).cancellations[0].id;ok(h.request({action:'cancelAcknowledge',k:'synthetic-link-a',cancellationId:id}));assert.ok(relief(h,id,'').error);ok(relief(h,id));ok(relief(h,id));assert.equal(bill(h).canBill,false);assert.match(bill(h).reason,/審査/);assert.ok(h.admin('kanriAddPayment',{studentId:'test-a',ym:'2026-09',requestId:'pending-relief-invoice',silent:true}).error);
 const review={studentId:'test-a',cancellationId:id,requestId:'relief-request-01',amount:500,note:'急病のため減額します'};assert.ok(h.request({action:'admin',op:'serviceCancelReliefReview',...review}).error);assert.ok(h.admin('serviceCancelReliefReview',{...review,studentId:'test-b'}).error);assert.ok(h.admin('serviceCancelReliefReview',{...review,amount:1001}).error);ok(h.admin('serviceCancelReliefReview',review));ok(h.admin('serviceCancelReliefReview',review));assert.ok(h.admin('serviceCancelReliefReview',{...review,amount:0}).error);assert.equal(bill(h).amount,500);assert.equal(bill(h).canBill,true);const state=h.get({action:'state',k:'synthetic-link-a'});assert.equal(state.cancellations[0].confirmed,false);assert.equal(state.cancellations[0].relief.status,'reduced');assert.equal(h.rows('slots').length,0);
});
test('issued invoice cannot be changed by relief review',()=>{const h=fixture();h.advance(10*3600000+1);request(h);ok(decide(h,quote(h)));const id=h.get({action:'state',k:'synthetic-link-a'}).cancellations[0].id;const inv=ok(h.admin('kanriAddPayment',{studentId:'test-a',ym:'2026-09',requestId:'relief-issued-invoice',silent:true})).invoice;ok(relief(h,id));const review={studentId:'test-a',cancellationId:id,requestId:'relief-request-01',amount:0,note:'免除'};assert.ok(h.admin('serviceCancelReliefReview',review).error);ok(h.admin('kanriVoidInvoice',{studentId:'test-a',invoiceId:inv.id,reason:'減免申請'}));ok(h.admin('serviceCancelReliefReview',review));assert.equal(bill(h).amount,0);});

test('family relief is scoped to linked child and monthly close waits for review',()=>{
 const h=fixture();h.advance(10*3600000+1);request(h);ok(decide(h,quote(h)));const id=h.get({action:'state',k:'synthetic-link-a'}).cancellations[0].id;
...
test('Worker D1 persists relief request, billing hold and teacher decision',async()=>{
 await (await import('../scripts/build-gas-bundle.mjs')).generate();const h=fixture();h.advance(10*3600000+1);request(h);ok(decide(h,quote(h)));const id=h.get({action:'state',k:'synthetic-link-a'}).cancellations[0].id;
 const {createParity}=require('./helpers/parity-harness.cjs'),p=await createParity(h),{runWrite}=await import('../cf/worker/write.mjs');
 try{const opts={now:h.now()},auth={action:'admin',token:require('./gas-harness.cjs').TEACHER_TOKEN,studentId:'test-a'};
 ok((await runWrite({action:'cancelReliefRequest',k:'synthetic-link-a',cancellationId:id,requestId:'worker-relief-01',reason:'illness'},p.env,opts)).result);
 assert.ok((await runWrite({...auth,op:'kanriAddPayment',ym:'2026-09',requestId:'worker-hold-01',silent:true},p.env,opts)).result.error);
 ok((await runWrite({...auth,op:'serviceCancelReliefReview',cancellationId:id,requestId:'worker-relief-01',amount:500,note:'reduced'},p.env,opts)).result);
 const inv=ok((await runWrite({...auth,op:'kanriAddPayment',ym:'2026-09',requestId:'worker-released-01',silent:true},p.env,opts)).result);assert.equal(inv.invoice.fees[0].amount,500);
 }finally{p.d1._sqlite.close();}
});
