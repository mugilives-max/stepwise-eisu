...
test('the admin plan card lists lines with status, opens one editor at a time, and sends line operations with the line id and revision', async () => {
  ...
  assert.match(ui.html(), /<tr><th>科目<\/th><th>種類<\/th><th>回数<\/th><th>期間<\/th><th>時間<\/th><th>1回の授業料<\/th><th><\/th><\/tr><tr class="plan-row" data-line="p1"><td>英語<\/td><td>通常<\/td><td>4回<\/td><td>2026年9月<\/td><td>90分<\/td><td>3,000円<\/td><td class="plan-act"><button class="btn-quiet btn-sm" data-action="pe-open" data-line="p1"[^>]*>✎<\/button><\/td><\/tr>/);
  assert.match(ui.html(), /<tr class="plan-row" data-line="a1"><td>数学<\/td><td>演習<\/td><td>4回<br><span class="small muted">承認 3回<\/span><\/td><td>2026\/9\/22〜10\/5<\/td><td>60分<\/td><td>3,000円<\/td><td class="plan-act">/); assert.match(ui.html(), /承諾: 2026-09-05・LINE/);
  assert.doesNotMatch(ui.html(), /class="tag (amber|gray)">(承認待ち|下書き)</); assert.match(ui.html(), /<tr class="plan-row" data-line="d1"><td>国語<\/td>/); assert.match(ui.html(), /<td class="plan-act"><button class="btn-primary btn-sm" data-action="pl-send" data-line="d1" data-rev="1">送信<\/button><button class="btn-quiet btn-sm" data-action="pe-open" data-line="d1"/); assert.doesNotMatch(ui.html(), /案内を送信<\/button>|data-action="pl-delete"/);
