...
test('the admin plan card lists lines with status, opens one editor at a time, and sends line operations with the line id and revision', async () => {
  ...
  assert.match(ui.html(), /<div class="plan-grid"><div class="plan-gh">科目<span[^>]*>種類<\/span><\/div><div class="plan-gh">回数<\/div><div class="plan-gh">期間<\/div><div class="plan-gh">時間<span[^>]*>1回の料金<\/span><\/div><div class="plan-gr" data-line="p1"><div class="plan-gc">英語<span class="sub">通常<\/span><\/div><div class="plan-gc">4回<\/div><div class="plan-gc">9月<\/div><div class="plan-gc">90分<span class="sub">3,000円<\/span><\/div><\/div><div class="plan-gs" data-line="p1">[^]*?<button class="btn-quiet btn-sm" data-action="pe-open" data-line="p1"[^>]*>✎<\/button><button class="btn-quiet btn-sm" data-action="plancopy" data-line="p1">/);
  assert.match(ui.html(), /<div class="plan-gr" data-line="a1"><div class="plan-gc">数学<span class="sub">演習<\/span><\/div><div class="plan-gc">4回<br><span class="small muted">承認 3回<\/span><\/div><div class="plan-gc">9\/22〜<wbr>10\/5<\/div><div class="plan-gc">60分<span class="sub">3,000円<\/span><\/div><\/div>/); assert.match(ui.html(), /承諾: 2026-09-05・LINE/);
  ...
  assert.match(ui.html(), /<h2>授業計画の作成<\/h2><div class="card">[^]*?<h3 class="plan-group"[^>]*>下書き <span[^>]*>1件<\/span><\/h3>[^]*data-line="d1"[^]*毎月の既定回数[^]*<h2>授業計画の承認状況<\/h2><div class="card"><h3 class="plan-group"[^>]*>送信済み（保護者の承認待ち） <span[^>]*>1件<\/span><\/h3>[^]*data-line="p1"[^]*<h3 class="plan-group"[^>]*>承認済み <span[^>]*>1件<\/span><\/h3>[^]*data-line="a1"/); assert.doesNotMatch(ui.html(), /plan-group[^>]*>見送り/);
