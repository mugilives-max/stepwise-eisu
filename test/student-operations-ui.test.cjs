...
test('the student page has no 予定 tab and registers or removes schedule items from the selected day', async () => {
  ...
  ui.click('calday', { 'data-date':'2026-09-16' }); assert.match(ui.html(), /授業不可<\/span><span class="time">終日<\/span><span class="who">部活</); assert.match(ui.html(), /class="calday ngday(?: sel)?" data-action="calday" data-date="2026-09-16">16<span class="calmarks"><\/span><span class="callbl to"[^>]*>授業不可<\/span>/); ui.click('delblock', { 'data-ids':'b1' }); assert.deepEqual(ui.requests.at(-1).body, { action:'unblock', k:'test-link-a', blockIds:['b1'] });
  ui.requests.at(-1).reply({ ok:true, state:s }); await flush();
