...
test('students turn a sentence into checked proposals and register them through the existing actions', async () => {
  ...
  assert.doesNotMatch(ui.html(), /文章で自動入力|手動で予定入力|data-action="dayact"/); assert.match(ui.html(), /<strong>文章を書くだけで、AIが「授業できる時間帯」「授業できない日」「予定の共有」に分けて登録の下書きを作ります。<\/strong>[^<]*チェックを入れた項目だけ登録されます/); assert.match(ui.html(), /data-action="dayadd"[^>]*aria-expanded="true">閉じる<\/button>/);
