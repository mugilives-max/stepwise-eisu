...
function stepwiseNativeLessonCheck() {
  ...
  try {
    ...
    app.getSheetByName('slots').appendRow(['native-next-lesson','2026-09-11','15:00',60,'booked',studentId,false,'','','英語','']);
    ...
    ensureLessonSchema_();
    check(app.getSheetByName('tasks').getLastColumn()===16,'task_deadline_schema');
    var baseline=unchanged();
    var req={slotId:'native-lesson-slot',expectedRevision:0,requestId:'native-save-1',record:{content:'=1+1\n<script>文字列</script>',progress:'基礎を確認',nextFocus:'次回の基本問題',teacherNote:"'NATIVE_PRIVATE_SENTINEL",homework:[{itemId:'native-item-1',title:'=1+1',due:'2026-09-10',type:'宿題'},{itemId:'native-item-2',title:'問題2',due:'',dueMode:'nextLesson',type:'宿題'}]}};
    ...
    check(lessonRows_('tasks').length===0,'save_does_not_create_tasks');
    var published=lessonPublishedForStudent_(studentId);
    check(published.length===1 && published[0].content===req.record.content,'saved_record_is_public');
    check(JSON.stringify(published).indexOf('NATIVE_PRIVATE_SENTINEL')<0,'public_record_excludes_private_note');
    check(!app.getSheetByName('lessonPublicSnapshots').getDataRange().getFormulas().some(function (row) { return row.some(Boolean); }),'no_public_snapshot_formulas');
    check(published[0].homework[1].due==='2026-09-11' && published[0].homework[1].dueStart==='15:00','public_next_lesson_deadline');
    ...
    var nextTask=lessonRows_('tasks').filter(function (t) { return t.sourceItemId==='native-item-2'; })[0];
    check(lessonTaskDueView_(nextTask).due==='2026-09-11','applied_next_lesson_deadline');
    check(lessonSetTaskDone_(nextTask,true).ok,'next_lesson_completion');
    nextTask=lessonFind_('tasks','id',nextTask.id);
    check(normDate_(nextTask.due)==='2026-09-11' && nextTask.dueTime==='15:00' && !!nextTask.doneAt,'completion_freezes_deadline');
    ...
    check(JSON.stringify(lessonPublishedForStudent_(studentId)).indexOf('未公開の下書き')<0,'report_draft_stays_private');
