...
test('new saves publish whitelisted snapshots to only the original student and omit private drafts',()=>{
  ...
  assert.deepEqual(Object.keys(h.public()[0]).sort(),['recordId','revision','date','start','min','subject','content','progress','nextFocus','publishedAt','homework','report','workspace','outline'].sort());
  assert.equal(h.public()[0].outline,null,'legacy records have no inferred position');
