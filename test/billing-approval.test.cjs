'use strict';

const test = require('node:test');
const assert = require('node:assert/strict');
const { createBillingHarness, MCP_KEY } = require('./helpers/billing-harness.cjs');
const { failWriteOnce } = require('./helpers/scheduling-harness.cjs');
const parentSession = require('./helpers/parent-session.cjs');

function ok(result) {
  assert.equal(result.ok, true, JSON.stringify(result));
  return result;
}

function rejected(result, code) {
  assert.ok(result && result.error, JSON.stringify(result));
  assert.notEqual(result.ok, true);
  if (code) assert.equal(result.errorCode, code, JSON.stringify(result));
  return result;
}

const json = value => JSON.parse(JSON.stringify(value));
const monthEnd = ym => { const [y, m] = ym.split('-').map(Number); return ym + '-' + String(new Date(y, m, 0).getDate()).padStart(2, '0'); };
const EVIDENCE = { via: '電話', consentDate: '2026-09-06', memo: '架空の保護者から承諾を得た記録' };

function preview(h, studentId = 'test-a', ym = '2026-09') {
  return ok(h.admin('billingPreview', { studentId, ym })).billing;
}

// Lines replace the old per-month plan: one line = subject × kind × period → count, duration and fee.
function proposeLine(h, { studentId = 'test-a', ym = '2026-09', subject = '数学', kind = '通常', count = 2, lessonMin = 90, lessonFee = 4500, comment = '', ...rest } = {}) {
  return ok(h.admin('planLineSave', { studentId, subject, kind, count, startDate: ym + '-01', endDate: monthEnd(ym), lessonMin, lessonFee, comment, propose: true, ...rest })).line;
}

function approveLine(h, line, studentId = 'test-a', evidence = {}) {
  return ok(h.admin('planLineApproveTeacher', { studentId, lineId: line.id, expectedRevision: line.revision, ...EVIDENCE, ...evidence })).line;
}

function approveMonth(h, options = {}) {
  return approveLine(h, proposeLine(h, options), options.studentId);
}

const lines = (h, studentId = 'test-a') => json(h.context().planLinesFor_(studentId));
const lineById = (h, id, studentId = 'test-a') => lines(h, studentId).find(l => l.id === id);
const editLine = (h, line, patch = {}) => h.admin('planLineSave', {
  studentId: 'test-a', lineId: line.id, subject: line.subject, kind: line.kind || '通常', count: line.count, startDate: line.startDate, endDate: line.endDate,
  lessonMin: line.lessonMin, lessonFee: Math.round(line.rate30 * line.lessonMin / 30), comment: line.comment, expectedRevision: line.revision, ...patch
});

test('line saves, approvals, fee edits and invoice creation require teacher authorization', () => {
  const h = createBillingHarness();
  const ptoken = parentSession(h);
  const line = proposeLine(h);
  const beforeStudents = h.rows('students');
  const beforeLines = h.rows('planLines');
  const operations = [
    { op: 'planLineSave', studentId: 'test-a', subject: '英語', kind: '通常', count: 8, startDate: '2026-09-01', endDate: '2026-09-30', lessonMin: 90, lessonFee: 4500 },
    { op: 'planLineApproveTeacher', studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, ...EVIDENCE },
    { op: 'planLineDelete', studentId: 'test-a', lineId: line.id },
    { op: 'setFee', studentId: 'test-a', rate30: 9999, monthly: 0 },
    { op: 'kanriAddPayment', studentId: 'test-a', ym: '2026-09', amount: 99999 }
  ];
  for (const operation of operations) {
    for (const credentials of [{}, { token: ptoken }, { mcpKey: MCP_KEY }]) {
      rejected(h.request({ action: 'admin', ...operation, ...credentials }));
    }
  }
  assert.deepEqual(h.rows('students'), beforeStudents);
  assert.deepEqual(h.rows('planLines'), beforeLines);
  assert.deepEqual(h.payments(), []);
  assert.deepEqual(h.effects, []);
});

test('legacy parent line decision cannot use an absent or another student session', () => {
  const h = createBillingHarness();
  const ptoken = parentSession(h);
  const line = proposeLine(h);
  proposeLine(h, { studentId: 'test-b' });
  const beforeLines = h.rows('planLines');
  for (const credentials of [{}, { k: 'synthetic-link-b', ptoken }]) {
    const result = rejected(h.parent('parentPlanDecide', { lineId: line.id, expectedRevision: line.revision, approve: true, ...credentials }));
    assert.equal(result.parentAuthRequired, true);
  }
  assert.deepEqual(h.rows('planLines'), beforeLines);
  assert.deepEqual(h.payments(), []);
});

test('a teacher can send an offer before a line is approved without confirming or billing it', () => {
  const h = createBillingHarness();
  const result = ok(h.admin('offer', { studentId: 'test-a', date: '2026-09-15', start: '16:00', min: 60, subject: '数学' }));
  assert.equal(result.added, 1);
  const slots = h.rows('slots');
  assert.equal(slots.length, 1);
  assert.equal(slots[0].status, 'offered');
  assert.ok(!slots[0].done);
  assert.deepEqual(h.rows('planLines'), []);
  assert.deepEqual(h.payments(), []);
  assert.deepEqual(h.effects, []);
});

test('an offer cannot be accepted without an approved line for its student, subject and date', () => {
  for (const setup of [
    () => {},
    h => h.seedPlan({ ym: 'default', status: 'approved' }),
    h => approveMonth(h, { studentId: 'test-b' }),
    h => approveMonth(h, { ym: '2026-10' }),
    h => approveMonth(h, { subject: '英語' }),
    h => approveMonth(h, { endDate: '2026-09-14' }),
    h => proposeLine(h)
  ]) {
    const h = createBillingHarness();
    setup(h);
    const slot = h.seedSlot();
    rejected(h.accept(slot.id), 'approvalRequired');
    assert.equal(h.rows('slots').find(row => row.id === slot.id).status, 'offered');
    assert.deepEqual(h.payments(), []);
    assert.deepEqual(h.effects, []);
  }
});

test('an approved line quota counts each booked lesson once and cancellation releases a place', () => {
  const h = createBillingHarness();
  h.seedSlot({ date: '2026-09-01', status: 'booked', done: true });
  approveMonth(h, { count: 2 });
  const first = h.seedSlot({ date: '2026-09-15' });
  const second = h.seedSlot({ date: '2026-09-22' });
  ok(h.accept(first.id));
  rejected(h.accept(second.id), 'planLimit');
  ok(h.admin('unbook', { studentId: 'test-a', slotId: first.id }));
  ok(h.accept(second.id));
  assert.equal(h.rows('slots').filter(row => row.status === 'booked').length, 2);
});

test('parent approval requires the displayed line revision and only approves the current own line', () => {
  const h = createBillingHarness();
  const ptoken = parentSession(h);
  const old = proposeLine(h);
  proposeLine(h, { studentId: 'test-b', subject: '英語', count: 1 });
  const current = ok(editLine(h, old, { count: 3, propose: true })).line;
  assert.notEqual(current.revision, old.revision);
  for (const revision of [undefined, old.revision]) {
    rejected(h.request({ action: 'familyPlanDecide', studentId: 'test-a', ftoken: ptoken, lineId: old.id, approve: true, expectedRevision: revision }), 'conflict');
  }
  assert.equal(preview(h).planStatus, 'proposed');
  ok(h.request({ action: 'familyPlanDecide', studentId: 'test-a', ftoken: ptoken, lineId: old.id, approve: true, expectedRevision: current.revision }));
  assert.equal(preview(h).planStatus, 'approved');
  assert.equal(lineById(h, old.id).approvedCount, 3);
  assert.equal(preview(h, 'test-b').planStatus, 'proposed');
  assert.deepEqual(h.effects, []);
});

test('changing one line invalidates only that line until it is approved again', () => {
  const h = createBillingHarness();
  const math = approveMonth(h, { count: 2 });
  approveMonth(h, { subject: '英語', count: 2 });
  h.seedSlot({ date: '2026-09-01', status: 'booked', done: true });
  assert.equal(preview(h).canBill, true);
  const edited = ok(editLine(h, math, { count: 3 }));
  assert.equal(edited.invalidated, true);
  assert.equal(edited.line.status, 'draft');
  assert.equal(edited.line.revision, math.revision + 1);
  assert.equal(preview(h).planStatus, 'draft');
  assert.equal(lineById(h, math.id).status, 'draft');
  const english = h.seedSlot({ subject: '英語' });
  ok(h.accept(english.id));
  rejected(h.accept(h.seedSlot({ date: '2026-09-16' }).id), 'approvalRequired');
  const blocked = preview(h);
  assert.equal(blocked.canBill, false);
  assert.equal(blocked.reason, '承認されていない科目・回数の授業があります');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-unapproved-line' }), 'approvalRequired');
  assert.deepEqual(h.payments(), []);
});

test('teacher approval records require a current line revision and consent evidence', () => {
  const h = createBillingHarness();
  const line = proposeLine(h);
  // A booked lesson preceding the consent date makes the record retrospective, so a
  // reason is required in addition to the date and method.
  h.seedSlot({ date: '2026-09-01', status: 'booked' });
  const valid = { studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, ...EVIDENCE };
  for (const field of ['expectedRevision', 'via', 'consentDate', 'memo']) {
    rejected(h.admin('planLineApproveTeacher', { ...valid, [field]: '' }));
    assert.equal(preview(h).planStatus, 'proposed');
  }
  rejected(h.admin('planLineApproveTeacher', { ...valid, consentDate: '2026-09-08' }));
  ok(h.admin('planLineApproveTeacher', valid));
  assert.equal(preview(h).planStatus, 'approved');
  const saved = lineById(h, line.id);
  assert.equal(saved.approvedVia, '電話');
  assert.equal(saved.consentDate, '2026-09-06');
  assert.equal(saved.memo, EVIDENCE.memo);
  assert.equal(saved.approvedCount, 2);
});

test('a line keeps its own fee after base rate changes and a re-proposed line is priced provisionally until approved', () => {
  const h = createBillingHarness();
  const line = approveMonth(h, { ym: '2026-08', lessonFee: 3000 });
  assert.equal(line.rate30, 1000);
  ok(h.admin('setFee', { studentId: 'test-a', rate30: 2500, monthly: 0 }));
  h.seedSlot({ date: '2026-08-10', status: 'booked', done: true });
  const bill = preview(h, 'test-a', '2026-08');
  assert.equal(bill.amount, 2000);
  assert.equal(bill.rate30, 1000);
  assert.equal(bill.canBill, true);
  assert.equal(bill.planStatus, 'approved');
  assert.equal(bill.revision, 0);
  assert.equal(bill.lessons[0].lineId, line.id);
  assert.equal(bill.lessons[0].lineRevision, line.revision);
  const resent = ok(editLine(h, line, { lessonFee: 3600, propose: true })).line;
  assert.equal(resent.revision, line.revision + 1);
  const changed = preview(h, 'test-a', '2026-08');
  assert.equal(changed.amount, 5000);
  assert.equal(changed.provisional, true);
  assert.equal(changed.planStatus, 'proposed');
  assert.equal(changed.canBill, false);
  approveLine(h, resent);
  const approved = preview(h, 'test-a', '2026-08');
  assert.equal(approved.amount, 2400);
  assert.equal(approved.provisional, false);
  assert.equal(approved.canBill, true);
  assert.equal(approved.lessons[0].lineRevision, resent.revision);
});

test('billing uses only completed lessons and rejects a month with unfinished bookings', () => {
  const h = createBillingHarness();
  approveMonth(h, { count: 3 });
  h.seedSlot({ date: '2026-09-01', min: 90, status: 'booked', done: true });
  const unfinished = h.seedSlot({ status: 'booked' });
  h.seedSlot({ date: '2026-09-22' });
  const blocked = preview(h);
  assert.equal(blocked.minutes, 90);
  assert.equal(blocked.amount, 4500);
  assert.equal(blocked.canBill, false);
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-unfinished' }));
  ok(h.admin('unbook', { studentId: 'test-a', slotId: unfinished.id }));
  const ready = preview(h);
  assert.equal(ready.canBill, true);
  assert.equal(ready.amount, 4500);
});

test('fixed monthly fees are rejected and no completed lessons produce no invoice', () => {
  const h = createBillingHarness();
  rejected(h.admin('setFee', { studentId: 'test-a', rate30: 1500, monthly: 12000 }));
  approveMonth(h);
  const bill = preview(h);
  assert.equal(bill.mode, 'time');
  assert.equal(bill.minutes, 0);
  assert.equal(bill.count, 0);
  assert.equal(bill.amount, 0);
  assert.equal(bill.canBill, false);
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-monthly-invoice' }));
  assert.equal(h.payments().length, 0);
});

function billableFixture() {
  const h = createBillingHarness();
  h.line = approveMonth(h, { count: 3 });
  h.seedSlot({ date: '2026-09-01', min: 90, status: 'booked', done: true });
  return h;
}

function issueInvoice(h, requestId = 'synthetic-invoice-request') {
  return ok(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId })).invoice;
}

test('an invoice ignores client calculation and saves the server terms and lesson snapshot', () => {
  const h = billableFixture();
  const base = { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invoice-request' };
  for (const forged of [{ amount: 1 }, { amount: 4501 }, { paidDate: '2026-09-01' }, { billDate: '2026-08-01' }]) {
    rejected(h.admin('kanriAddPayment', { ...base, ...forged }));
    assert.deepEqual(h.payments(), []);
  }
  rejected(h.admin('kanriAddPayment', { ...base, requestId: '' }), 'requestIdRequired');
  const invoice = issueInvoice(h);
  assert.equal(invoice.amount, 4500);
  assert.equal(invoice.revision, 0);
  const saved = h.payments()[0];
  assert.equal(saved['請求ID'], invoice.id);
  assert.equal(saved['確定単価(30分)'], 1500);
  assert.equal(saved['実施分数'], 90);
  assert.equal(saved['実施回数'], 1);
  assert.equal(saved['状態'], '未入金');
  assert.equal(Number(saved['承認版']), 0);
  const snapshot = JSON.parse(saved['実績JSON']);
  assert.equal(snapshot[0].min, 90);
  assert.equal(snapshot[0].lineId, h.line.id);
  assert.equal(snapshot[0].lineRevision, h.line.revision);
  assert.equal(snapshot[0].rate30, 1500);
  const issued = h.rows('approvalEvents').find(row => row.event === 'invoiced');
  assert.deepEqual(JSON.parse(issued.snapshotJson).plan.map(l => [l.id, l.revision, l.count, l.rate30]), [[h.line.id, h.line.revision, 3, 1500]]);
});

test('a reused invoice request cannot change its student, month or amount', () => {
  const h = billableFixture();
  issueInvoice(h);
  rejected(h.admin('kanriAddPayment', { studentId: 'test-b', ym: '2026-09', requestId: 'synthetic-invoice-request' }), 'conflict');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-10', requestId: 'synthetic-invoice-request' }), 'conflict');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invoice-request', amount: 1 }), 'conflict');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-another-request' }), 'invoiceExists');
  assert.equal(h.payments().length, 1);
});

// The issued audit snapshot is rebuilt from the ledger row (line refs + rate), so a retransmission audits identically.
test('invoice retransmission returns the same invoice, before and after it is voided', () => {
  const h = billableFixture();
  const first = issueInvoice(h);
  const replay = ok(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invoice-request' }));
  assert.equal(replay.replayed, true);
  assert.equal(replay.invoice.id, first.id);
  assert.equal(h.payments().length, 1);
  ok(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: first.id, reason: '架空の請求内容訂正' }));
  const voided = issueInvoice(h);
  assert.equal(voided.id, first.id);
  assert.equal(voided.status, '取消');
  assert.equal(h.payments().length, 1);
  assert.equal(h.rows('approvalEvents').filter(row => row.event === 'invoiced').length, 1);
});

test('legacy invoices block duplicates and schema extension preserves original business values', () => {
  const h = billableFixture();
  const original = h.seedPayment({ '請求額': 4321 });
  const before = Object.fromEntries(Object.keys(original).map(key => [key, original[key]]));
  const sheet = h.ledger.getSheetByName('入金管理');
  sheet.values = sheet.values.map(row => row.slice(0, 9));
  const legacyHeaders = [...sheet.values[0]];
  h.advance(86400000);
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-legacy-duplicate' }), 'invoiceExists');
  assert.equal(h.payments().length, 1);
  assert.deepEqual(sheet.values[0].slice(0, 9), legacyHeaders);
  assert.equal(sheet.values[0].length, 21);
  assert.ok(h.payments()[0]['請求ID']);
  for (const [key, value] of Object.entries(before)) assert.equal(h.payments()[0][key], value);
  rejected(h.admin('kanriDeleteRow', { studentId: 'test-a', sheet: '入金管理', row: 2 }));
  assert.equal(h.payments().length, 1);
});

test('issuing an invoice freezes its lines, booked lessons, completed status and amount', () => {
  const h = billableFixture();
  const completed = h.rows('slots')[0];
  const oldOffer = h.seedSlot({ date: '2026-09-02' });
  const futureOffer = h.seedSlot({ date: '2026-09-15' });
  issueInvoice(h);
  const before = { slots: h.rows('slots'), lines: h.rows('planLines'), payments: h.payments() };
  rejected(h.accept(futureOffer.id), 'invoiceLocked');
  for (const operation of [
    { op: 'toggleDone', slotId: completed.id },
    { op: 'unbook', slotId: completed.id },
    { op: 'finishOffered', slotId: oldOffer.id },
    { op: 'deleteSlot', slotId: oldOffer.id },
    { op: 'resolveCancel', slotId: completed.id, approve: true }
  ]) rejected(h.admin(operation.op, { studentId: 'test-a', ...operation }));
  rejected(editLine(h, h.line, { count: 4 }), 'invoiceLocked');
  rejected(editLine(h, h.line, { lessonFee: 6000, propose: true }), 'invoiceLocked');
  rejected(h.admin('planLineDelete', { studentId: 'test-a', lineId: h.line.id }), 'invoiceLocked');
  rejected(h.admin('planLineSave', { studentId: 'test-a', subject: '英語', kind: '通常', count: 1, startDate: '2026-09-20', endDate: '2026-10-05', lessonMin: 60, lessonFee: 3000 }), 'invoiceLocked');
  assert.deepEqual(h.rows('slots'), before.slots);
  assert.deepEqual(h.rows('planLines'), before.lines);
  assert.deepEqual(h.payments(), before.payments);
  ok(h.admin('setFee', { studentId: 'test-a', rate30: 2500, monthly: 0 }));
  assert.equal(preview(h).amount, 4500);
  assert.deepEqual(h.payments(), before.payments);
});

test('unpaid invoices can be voided with a reason and reissued under a new request without deleting history', () => {
  const h = billableFixture();
  const first = issueInvoice(h);
  rejected(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: first.id }));
  rejected(h.admin('kanriVoidInvoice', { studentId: 'test-b', invoiceId: first.id, reason: '架空の訂正' }), 'notFound');
  ok(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: first.id, reason: '架空の請求内容訂正' }));
  assert.equal(h.payments().length, 1);
  assert.equal(h.payments()[0]['状態'], '取消');
  assert.deepEqual(h.rows('approvalEvents').filter(row => row.event === 'invoiceVoided').map(row => row.memo), ['架空の請求内容訂正']);
  // The line is editable again once the month has no active invoice.
  ok(editLine(h, h.line, { count: 4, propose: true }));
  approveLine(h, lineById(h, h.line.id));
  const second = issueInvoice(h, 'synthetic-reissued-invoice');
  assert.notEqual(second.id, first.id);
  assert.equal(h.payments().length, 2);
  assert.equal(h.payments().filter(row => row['状態'] !== '取消').length, 1);
  assert.equal(JSON.parse(h.payments()[1]['実績JSON'])[0].lineRevision, h.line.revision + 1);
});

test('paid invoices require a recorded correction before voiding and use IDs instead of row positions', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  rejected(h.admin('kanriSetPaid', { studentId: 'test-b', invoiceId: invoice.id, expectedPaymentRevision: 0, date: '2026-09-07', method: '振込' }), 'notFound');
  rejected(h.admin('kanriSetPaid', { studentId: 'test-a', row: 2, expectedPaymentRevision: 0, date: '2026-09-07', method: '振込' }), 'notFound');
  ok(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 0, date: '2026-09-07', method: '振込' }));
  rejected(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: invoice.id, reason: '架空の訂正' }));
  rejected(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 1, unpaid: true }));
  assert.equal(h.payments()[0]['状態'], '入金済');
  ok(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 1, unpaid: true, reason: '架空の入金先取り違え訂正' }));
  assert.equal(h.payments()[0]['状態'], '未入金');
  assert.ok(h.rows('approvalEvents').some(row => row.event === 'paymentCorrected'));
  ok(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: invoice.id, reason: '架空の訂正' }));
  rejected(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 2, date: '2026-09-07', method: '振込' }));
});

test('teacher completion is independent of parent line approval and quota', () => {
  const h = createBillingHarness();
  const offered = h.seedSlot({ date: '2026-09-01' });
  const booked = h.seedSlot({ date: '2026-09-02', status: 'booked' });
  ok(h.admin('finishOffered', { studentId: 'test-a', slotId: offered.id }));
  ok(h.admin('toggleDone', { studentId: 'test-a', slotId: booked.id, done: true }));
  approveMonth(h, { count: 1 });
  ok(h.admin('toggleDone', { studentId: 'test-a', slotId: booked.id, done: true }));
  assert.equal(h.rows('slots').find(row => row.id === offered.id).status, 'booked');
  assert.equal(h.rows('slots').find(row => row.id === booked.id).done, true);
});

test('a ledger write failure before saving can be retried without creating duplicate invoices', () => {
  const h = billableFixture();
  const sheet = h.ledger.getSheetByName('入金管理');
  const append = sheet.appendRow;
  sheet.appendRow = function(row) {
    this.appendRow = append;
    throw new Error('Synthetic ledger service failure before append');
  };
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invoice-request' }));
  assert.equal(h.payments().length, 0);
  issueInvoice(h);
  assert.equal(h.payments().length, 1);
});

test('loss of the response after a durable invoice write does not duplicate the invoice on retry', () => {
  const h = billableFixture();
  failWriteOnce(h.spreadsheet.getSheetByName('approvalEvents'), values => values[0]?.[4] === 'invoiced');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invoice-request' }));
  assert.equal(h.payments().length, 1);
  const id = h.payments()[0]['請求ID'];
  assert.equal(h.rows('approvalEvents').filter(row => row.event === 'invoiced').length, 0);
  assert.equal(issueInvoice(h).id, id);
  assert.equal(h.payments().length, 1);
  const issuedEvents = h.rows('approvalEvents').filter(row => row.event === 'invoiced');
  assert.equal(issuedEvents.length, 1);
  assert.equal(JSON.parse(issuedEvents[0].snapshotJson).detail.invoiceId, id);
  issueInvoice(h);
  assert.equal(h.rows('approvalEvents').filter(row => row.event === 'invoiced').length, 1);
});

test('line schema validates headers before accepting mutations and preserves conflicting headers', () => {
  const h = createBillingHarness();
  const sheet = h.spreadsheet.getSheetByName('planLines');
  sheet.values[0][3] = 'unexpectedExistingColumn';
  const before = sheet.values.map(row => [...row]);
  h.advance(86400000); // Expire cached schema checks without depending on a cache key.
  rejected(h.admin('planLineSave', { studentId: 'test-a', subject: '数学', kind: '通常', count: 2, startDate: '2026-09-01', endDate: '2026-09-30', lessonMin: 90, lessonFee: 4500 }));
  assert.deepEqual(sheet.values, before);
  assert.deepEqual(h.rows('approvalEvents'), []);
});

test('invalid completed lesson durations cannot offset other lessons or become a billable snapshot', () => {
  for (const min of [-30, 0, Infinity, 'not-a-duration']) {
    const h = billableFixture();
    h.seedSlot({ date: '2026-09-02', min, status: 'booked', done: true });
    const bill = preview(h);
    assert.equal(bill.canBill, false, 'Invalid duration was billable: ' + String(min));
    assert.ok(bill.reason);
    rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-invalid-duration' }));
    assert.deepEqual(h.payments(), []);
  }
});

test('invoice amounts agree across parent, teacher and MCP views after standard fee changes', () => {
  const h = billableFixture();
  const ptoken = parentSession(h);
  issueInvoice(h);
  ok(h.admin('setFee', { studentId: 'test-a', rate30: 2900, monthly: 0 }));
  const teacher = ok(h.admin('kanriStudent', { studentId: 'test-a' })).data;
  const parent = ok(h.request({ action: 'familyData', studentId: 'test-a', ftoken: ptoken })).data;
  const dashboard = ok(h.admin('kanriDashboard')).data;
  const mcp = ok(h.mcp('mcpBilling', { month: '2026-09' }));
  assert.equal(teacher.thisMonth.fee, 4500);
  assert.equal(teacher.billing.amount, 4500);
  assert.equal(parent.thisMonth.fee, 4500);
  assert.equal(dashboard.students.find(student => student.id === 'test-a').feeThisMonth, 4500);
  assert.equal(mcp.rows.find(row => row.student.id === 'test-a').fee, 4500);
  assert.equal(mcp.rows.find(row => row.student.id === 'test-a').canBill, false);
});

test('void invoices remain in history but disappear from teacher and MCP outstanding lists', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  ok(h.admin('kanriVoidInvoice', { studentId: 'test-a', invoiceId: invoice.id, reason: '架空の請求取消' }));
  const dashboard = ok(h.admin('kanriDashboard')).data;
  const pending = ok(h.mcp('mcpPending'));
  const student = ok(h.mcp('mcpStudent', { studentId: 'test-a' }));
  assert.deepEqual(dashboard.unpaid, []);
  assert.deepEqual(pending.unpaid, []);
  assert.deepEqual(student.paymentsUnpaid, []);
  assert.equal(h.payments().length, 1);
});

test('duplicate line rows fail closed for approval and deletion', () => {
  const h = createBillingHarness();
  const line = proposeLine(h);
  const sheet = h.spreadsheet.getSheetByName('planLines');
  sheet.appendRow([...sheet.values[1]]);
  const before = h.rows('planLines');
  rejected(h.admin('planLineApproveTeacher', { studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, ...EVIDENCE }));
  rejected(h.admin('planLineDelete', { studentId: 'test-a', lineId: line.id }));
  assert.deepEqual(h.rows('planLines'), before);
  assert.deepEqual(h.rows('approvalEvents').map(row => row.event), ['proposed']);
});

// Two rows sharing a line id make planLinesFor_ throw, so edits, booking and invoicing all fail closed.
test('duplicate line rows fail closed for line edits, booking and invoicing', () => {
  const h = createBillingHarness();
  const line = approveMonth(h);
  h.seedSlot({ date: '2026-09-01', status: 'booked', done: true });
  const slot = h.seedSlot();
  const sheet = h.spreadsheet.getSheetByName('planLines');
  sheet.appendRow([...sheet.values[1]]);
  const before = { slots: h.rows('slots'), lines: h.rows('planLines') };
  rejected(editLine(h, line, { count: 3 }));
  rejected(h.accept(slot.id));
  assert.equal(h.rows('slots').find(row => row.id === slot.id).status, 'offered');
  rejected(h.admin('kanriAddPayment', { studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-duplicate-line' }));
  assert.deepEqual(h.payments(), []);
  assert.deepEqual(h.rows('slots'), before.slots);
  assert.deepEqual(h.rows('planLines'), before.lines);
});

test('waiting for another mutation returns pending if the script lock cannot be acquired', () => {
  const h = billableFixture();
  const context = h.context();
  let released = false;
  context.LockService.getScriptLock = () => ({
    waitLock() { throw new Error('Synthetic lock timeout'); },
    releaseLock() { released = true; }
  });
  const result = JSON.parse(context.doPost({ postData: { contents: JSON.stringify({
    action: 'admin', op: 'kanriAddPayment', studentId: 'test-a', ym: '2026-09', requestId: 'synthetic-lock-timeout'
  }) } }).getContent());
  rejected(result, 'pending');
  assert.equal(released, false);
  assert.deepEqual(h.payments(), []);
});

test('duplicate slot IDs reject confirmation and destructive mutations without picking a row', () => {
  const h = createBillingHarness();
  approveMonth(h, { count: 1 });
  const slot = h.seedSlot();
  h.seedSlot({ id: slot.id, status: 'booked' });
  const before = h.rows('slots');
  rejected(h.accept(slot.id), 'conflict');
  for (const op of ['toggleDone', 'finishOffered', 'unbook', 'deleteSlot']) {
    rejected(h.admin(op, { studentId: 'test-a', slotId: slot.id }), 'conflict');
  }
  assert.deepEqual(h.rows('slots'), before);
});

test('approval recovery rejects changed consent evidence and resumes the original evidence once', () => {
  const h = createBillingHarness();
  const line = proposeLine(h);
  const originalRequest = { studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, via: 'LINE', consentDate: '2026-09-06', memo: '架空の最初の承諾記録' };
  // The consent receipt (approvalEvents) is written before the line row; interrupt the line row write.
  failWriteOnce(h.spreadsheet.getSheetByName('planLines'), values => values[0]?.[10] === 'approved');
  rejected(h.admin('planLineApproveTeacher', originalRequest));
  assert.equal(preview(h).planStatus, 'proposed');
  assert.equal(lineById(h, line.id).status, 'proposed');
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'approved').length, 1);
  for (const changed of [{ via: '電話' }, { consentDate: '2026-09-05' }, { memo: '架空の別の承諾記録' }]) {
    rejected(h.admin('planLineApproveTeacher', { ...originalRequest, ...changed }), 'conflict');
    assert.equal(preview(h).planStatus, 'proposed');
  }
  ok(h.admin('planLineApproveTeacher', originalRequest));
  assert.equal(preview(h).planStatus, 'approved');
  const saved = lineById(h, line.id);
  const audits = h.rows('approvalEvents').filter(event => event.event === 'approved');
  assert.equal(audits.length, 1);
  assert.equal(audits[0].id, `${line.id}:${line.revision}:approved`);
  assert.equal(saved.approvedVia, audits[0].via);
  assert.equal(saved.consentDate, audits[0].consentDate);
  assert.equal(saved.memo, audits[0].memo);
  assert.equal(ok(h.admin('planLineApproveTeacher', originalRequest)).replayed, true);
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'approved').length, 1);
});

test('void recovery rejects a changed reason and resumes the original cancellation once', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  const originalRequest = { studentId: 'test-a', invoiceId: invoice.id, reason: '架空の最初の取消理由' };
  failWriteOnce(h.ledger.getSheetByName('入金管理'), values => values[0]?.[7] === '取消');
  rejected(h.admin('kanriVoidInvoice', originalRequest));
  assert.equal(h.payments()[0]['状態'], '未入金');
  rejected(h.admin('kanriVoidInvoice', { ...originalRequest, reason: '架空の別の取消理由' }), 'conflict');
  assert.equal(h.payments()[0]['状態'], '未入金');
  ok(h.admin('kanriVoidInvoice', originalRequest));
  const audits = h.rows('approvalEvents').filter(event => event.event === 'invoiceVoided');
  assert.equal(audits.length, 1);
  assert.equal(h.payments()[0]['状態'], '取消');
  assert.equal(h.payments()[0]['取消理由'], JSON.parse(audits[0].snapshotJson).detail.reason);
});

test('a stale payment update cannot overwrite paid date or method without a reasoned correction', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  const paidRequest = { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 0, date: '2026-09-06', method: '振込' };
  ok(h.admin('kanriSetPaid', paidRequest));
  for (const changed of [{ date: '2026-09-07' }, { method: '現金' }]) {
    rejected(h.admin('kanriSetPaid', { ...paidRequest, ...changed }), 'conflict');
  }
  assert.equal(h.payments()[0]['入金日'], '2026-09-06');
  assert.equal(h.payments()[0]['入金方法'], '振込');
  assert.equal(ok(h.admin('kanriSetPaid', paidRequest)).replayed, true);
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'paid').length, 1);
  ok(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 1, unpaid: true, reason: '架空の入金情報訂正' }));
  ok(h.admin('kanriSetPaid', { ...paidRequest, expectedPaymentRevision: 2, date: '2026-09-07', method: '現金' }));
  assert.equal(h.payments()[0]['入金日'], '2026-09-07');
  assert.equal(h.payments()[0]['入金方法'], '現金');
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'paymentCorrected').length, 1);
});

test('an old correction retry cannot erase a later payment even when its date and method are identical', () => {
  for (const replacement of [{ date: '2026-09-07', method: '現金' }, { date: '2026-09-06', method: '振込' }]) {
    const h = billableFixture();
    const invoice = issueInvoice(h);
    assert.equal(invoice.paymentRevision, 0);
    const base = { studentId: 'test-a', invoiceId: invoice.id };
    ok(h.admin('kanriSetPaid', { ...base, expectedPaymentRevision: 0, date: '2026-09-06', method: '振込' }));
    const oldCorrection = { ...base, expectedPaymentRevision: 1, unpaid: true, reason: '架空の最初の訂正・応答紛失' };
    ok(h.admin('kanriSetPaid', oldCorrection));
    assert.equal(ok(h.admin('kanriSetPaid', oldCorrection)).replayed, true);
    ok(h.admin('kanriSetPaid', { ...base, expectedPaymentRevision: 2, ...replacement }));
    rejected(h.admin('kanriSetPaid', oldCorrection), 'conflict');
    assert.equal(h.payments()[0]['状態'], '入金済');
    assert.equal(h.payments()[0]['入金日'], replacement.date);
    assert.equal(h.payments()[0]['入金方法'], replacement.method);
    assert.equal(Number(h.payments()[0]['入金版']), 3);
    assert.equal(h.rows('approvalEvents').filter(event => event.event === 'paymentCorrected').length, 1);
  }
});

test('payment mutations require an explicit valid version, including no-op retransmissions', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  const base = { studentId: 'test-a', invoiceId: invoice.id, date: '2026-09-06', method: '振込' };
  for (const expectedPaymentRevision of [undefined, null, '', -1, 0.5]) {
    rejected(h.admin('kanriSetPaid', { ...base, expectedPaymentRevision }), 'conflict');
  }
  assert.equal(h.payments()[0]['状態'], '未入金');
  ok(h.admin('kanriSetPaid', { ...base, expectedPaymentRevision: 0 }));
  rejected(h.admin('kanriSetPaid', base));
  rejected(h.admin('kanriSetPaid', { studentId: 'test-a', invoiceId: invoice.id, unpaid: true, reason: '架空の訂正理由' }));
  assert.equal(h.payments()[0]['状態'], '入金済');
  assert.equal(Number(h.payments()[0]['入金版']), 1);
  const teacher = ok(h.admin('kanriStudent', { studentId: 'test-a' })).data;
  assert.equal(teacher.payments.find(payment => payment.id === invoice.id).paymentRevision, 1);
});

test('payment recovery preserves its version and audit evidence after the final row write fails', () => {
  const h = billableFixture();
  const invoice = issueInvoice(h);
  const request = { studentId: 'test-a', invoiceId: invoice.id, expectedPaymentRevision: 0, date: '2026-09-06', method: '振込' };
  failWriteOnce(h.ledger.getSheetByName('入金管理'), values => values[0]?.[7] === '入金済');
  rejected(h.admin('kanriSetPaid', request));
  assert.equal(h.payments()[0]['状態'], '未入金');
  assert.equal(Number(h.payments()[0]['入金版']), 0);
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'paid').length, 1);
  rejected(h.admin('kanriSetPaid', { ...request, method: '現金' }), 'conflict');
  assert.equal(h.payments()[0]['状態'], '未入金');
  ok(h.admin('kanriSetPaid', request));
  assert.equal(h.payments()[0]['状態'], '入金済');
  assert.equal(h.payments()[0]['入金方法'], '振込');
  assert.equal(Number(h.payments()[0]['入金版']), 1);
  assert.equal(ok(h.admin('kanriSetPaid', request)).replayed, true);
  assert.equal(h.rows('approvalEvents').filter(event => event.event === 'paid').length, 1);
});

test('duration is saved with the line and reduced parent counts govern actual booking limits', () => {
  const h = createBillingHarness();
  const line = proposeLine(h, { count: 4, lessonMin: 90, lessonFee: 4500 });
  const token = parentSession(h);
  const req = { action: 'familyPlanDecide', ftoken: token, studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, approve: true, approvedCount: 2, memo: '2回まで' };
  const r = ok(h.request(req));
  assert.equal(r.data.planLines[0].lessonMin, 90);
  assert.equal(r.data.planLines[0].approvedCount, 2);
  assert.equal(r.data.planLines[0].count, 4);
  assert.equal(Number(h.rows('planLines')[0].lessonMin), 90);
  assert.equal(Number(h.rows('planLines')[0].approvedCount), 2);
  assert.equal(ok(h.request(req)).replayed, true);
  rejected(h.request({ ...req, approvedCount: 3 }), 'conflict');
  const c = h.context();
  const sh = h.spreadsheet.getSheetByName('slots');
  for (let i = 0; i < 2; i++) {
    const slot = { id: 'limit-' + i, studentId: 'test-a', date: '2026-09-20', start: '10:00', min: 90, subject: '数学', status: 'booked' };
    sh.appendRow(sh.values[0].map(k => slot[k] ?? ''));
  }
  c.memoClear_();
  assert.equal(c.billingSlotAllowed_({ id: 'new-slot', studentId: 'test-a', date: '2026-09-21', start: '10:00', min: 90, subject: '数学' }).errorCode, 'planLimit');
  assert.equal(preview(h).planStatus, 'approved');
});

test('partial approval rejects invalid counts and zero counts remain declined', () => {
  const h = createBillingHarness();
  const line = proposeLine(h, { count: 4 });
  const token = parentSession(h);
  const req = { action: 'familyPlanDecide', ftoken: token, studentId: 'test-a', lineId: line.id, expectedRevision: line.revision, approve: false };
  for (const approvedCount of [-1, 5, 1.5]) rejected(h.request({ ...req, approvedCount }));
  assert.equal(h.rows('planLines')[0].status, 'proposed');
  const declined = ok(h.request({ ...req, approvedCount: 0 }));
  assert.equal(h.rows('planLines')[0].status, 'declined');
  assert.equal(Number(h.rows('planLines')[0].approvedCount), 0);
  assert.deepEqual(declined.data.planLines, []);
  assert.equal(preview(h).planStatus, 'declined');
  rejected(h.accept(h.seedSlot().id), 'approvalRequired');
});
