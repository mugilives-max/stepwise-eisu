...
test('teacher preview returns the student and parent views read-only with the teacher token and never the student code or email status',()=>{
 ...
 h.admin('familyCreate',{label:'【テスト】保護者',studentIds:['test-a']});
 ...
 const home=call({token:TEACHER_TOKEN,studentId:'test-a',view:'home'});assert.equal(home.ok,true);assert.deepEqual(JSON.parse(JSON.stringify(home.children.map(x=>x.studentId))),['test-a']);assert.equal(home.family.label,'【テスト】保護者');assert.ok(Array.isArray(home.billing));
