...
test('student permissions persist in D1 and enforce subsequent student writes',async()=>{const {h,ftoken}=familyLedger(),p=await createParity(h),{runWrite}=await import('../cf/worker/write.mjs');const r=await runWrite({action:'familyPermissionsSave',ftoken,studentId:'test-a',permission:'booking',allowed:false,expectedRevision:0},p.env,{now:h.now()});assert.equal(r.result.ok,true,JSON.stringify(r.result));const home=await p.worker({action:'familyHome',ftoken});assert.equal(home.children[0].permissions.booking,false);assert.equal(home.children[0].permissionsRevision,1);const denied=await runWrite({action:'accept',k:h.studentRow().code,familyProxy:true},p.env,{now:h.now()});assert.equal(denied.result.errorCode,'studentPermissionDenied');});
