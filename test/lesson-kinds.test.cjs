...
test('lines and offers carry the kind; 通常 stays empty in storage and billing matches subject and kind', () => {
  const h = createSchedulingHarness(); ok(h.admin('state'));
  ok(h.admin('lessonKindSave', { name: '演習', standardMin: 60 })); ok(h.admin('lessonKindSave', { name: '講習', active: false }));
  const line = (args) => h.admin('planLineSave', { studentId: 'test-a', subject: '英語', kind: '通常', count: 3, startDate: '2026-09-01', endDate: '2026-09-30', lessonMin: 90, lessonFee: 4500, comment: '', propose: true, ...args });
  rejected(line({ kind: '講習' })); rejected(line({ kind: 'なにか' }));
  const a = ok(line({})); const b = ok(line({ kind: '演習', count: 2, lessonMin: 60, lessonFee: 3000 }));
  h.approveLine(a.line.id); h.approveLine(b.line.id);
  const c = h.context();
  ...
  assert.deepEqual(h.rows('planLines').map(l => l.kind), ['', '演習']);
  // offers
  ...
  assert.deepEqual(state.planLines.map(l => [l.subject, l.kind, l.count, l.status]), [['英語', '', 3, 'approved'], ['英語', '演習', 2, 'approved']]);
