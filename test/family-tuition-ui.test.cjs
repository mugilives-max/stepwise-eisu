...
test('family tuition sums current cancellation amounts for the displayed month including pending relief',()=>{
 ...
 assert.match(html,new RegExp('<th scope="row">キャンセル料</th>'));assert.ok(html.includes('<td>1000円</td>'));assert.ok(html.includes('<td>4500円</td>'));assert.ok(!html.includes('小計'));assert.ok(!html.includes('<th>月</th>'));assert.ok(html.includes('2026年9月'));assert.ok(html.includes('rowspan="2"'));assert.ok(!html.includes('rowspan="3"'));assert.ok(!html.includes('>2026-09<'));assert.ok(html.includes('<td rowspan="2">a</td><th scope="row">授業</th>'));assert.ok(html.includes('<td>8500円</td>'));assert.ok(html.includes('<td>11500円</td>'));assert.ok(!html.includes('9000円'));assert.ok(html.includes('審査前の金額'));
});
