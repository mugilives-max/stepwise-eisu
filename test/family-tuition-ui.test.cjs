...
test('family tuition sums current cancellation amounts for the displayed month including pending relief',()=>{
 ...
 assert.match(html,new RegExp('<th>キャンセル料</th>'));assert.ok(html.includes('<td>1000円</td>'));assert.ok(html.includes('<td>4500円</td>'));assert.ok(html.includes('<td>5500円</td>'));assert.ok(!html.includes('9000円'));assert.ok(html.includes('審査前の金額'));
});
