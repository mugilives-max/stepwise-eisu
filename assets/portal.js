      ...
      (function () {
        ...
        function cancelReviewDetails(s){
          ...
          var rows=[['授業日時',s.date.replace(/-/g,'/')+' '+s.start+'〜'+endTime(s.start,s.min)],['科目',s.subject||'—'],['状態','キャンセル済み'],['連絡受付日時',stamp],['連絡方法',({request:'マイページからの申請',external:'LINE・電話など',noshow:'連絡なし（無断欠席）',teacher:'先生都合'})[s.source]||'記録なし'],['規定のキャンセル料',known?yen(standard):'記録なし'],['減額・免除',reduction?(amount===0?'全額免除':'減額')+'（'+yen(reduction)+'）':'なし'],['最終請求額',yen(amount)]];
