      ...
      (function () {
        ...
        function renderMonthSummary(D) {
          ...
          extraKeys.forEach(function(label){var n=extra[label],mm=/^(.*)（(.+)）$/.exec(label);shown++;var endDay=new Date(Number(ymNow.slice(0,4)),Number(ymNow.slice(5)),0).getDate();html+='<tr><td>'+Number(ymNow.slice(5))+'/1〜'+Number(ymNow.slice(5))+'/'+endDay+'</td><td>'+esc(mm?mm[1]:label)+'</td><td>'+esc(mm?mm[2]:'通常')+'</td><td>—</td><td>'+n.plan+'回</td><td>'+n.done+'回</td><td>計画外</td></tr>';});
