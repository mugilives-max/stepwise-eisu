      ...
      (function () {
        ...
        function renderFamily() {
          ...
          if (F.home && F.step === "home") {
            ...
            h += '<div class="card"><table class="family-info-table"><thead><tr><th scope="col">項目</th><th scope="col">登録内容</th><th scope="col">状態</th></tr></thead><tbody>'+infoRow('保護者の登録名',account.label)+infoRow('保護者の姓',account.familyName)+infoRow('保護者の名',account.givenName)+infoRow('保護者の連絡用メールアドレス',account.email,'メール確認済み')+(children.length?children.map(function(c,i){var who='生徒 '+(i+1)+'（'+c.name+'）';return infoRow(who+'の姓',c.familyName)+infoRow(who+'の名',c.givenName)+infoRow(who+'の連絡用メールアドレス',c.email,c.emailVerified?'メール確認済み':'メール未確認');}).join(''):infoRow('登録済みの生徒',''))+'</tbody></table><div class="row" style="margin-top:18px"><button class="btn-quiet btn-sm" data-action="fa-home"'+dis+'>家族情報を更新</button> <button class="btn-quiet btn-sm" data-action="fa-mode" data-step="emailChange"'+dis+'>メールアドレスを変更</button></div></div>';
