      ...
      (function () {
        ...
        function changeHistoryTable(changes){return (changes||[]).map(function(c){
          ...
          function lessonRow(label,v){return '<tr><th scope="row"'+cell+'>'+esc(label+'：'+(v.subject||'—')+'（'+(v.kind||'通常')+'）')+'</th>'+[v.date,v.start+'〜'+endTime(v.start,v.min)].map(function(value){return '<td'+cell+'>'+esc(value)+'</td>';}).join('')+'</tr>';}
